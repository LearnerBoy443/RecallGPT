...
class DatabaseStorage(Storage):
    def deconstruct(self):
        return ("api.storage.DatabaseStorage", [], {})

    def _open(self, name, mode='rb'):
        from .models import DatabaseFile
        try:
            db_file = DatabaseFile.objects.get(name=name)
            content_bytes = bytes(db_file.content) if isinstance(db_file.content, memoryview) else db_file.content
            return ContentFile(content_bytes, name=name)
