...
class ImageGenerationAPIView(APIView):
    ...
    def post(self, request):
        ...
        # FLUX models are natively photorealistic — pass the prompt as-is.
        # No keyword stuffing; descriptive natural-language prompts produce the best real-life results.

        image_content = None
        model_used = None
        is_fallback = False

        # Attempt to use Hugging Face models if token is configured and not direct pollinations
        if hf_token and model_choice != 'pollinations':
            from huggingface_hub import InferenceClient
            import io

            # Fallback chain (per project architecture):
            #   FLUX.1-schnell (primary) -> FLUX.1-dev (fallback 1) -> Stable Diffusion 3.5 Large (fallback 2) -> Stable Diffusion 1.5 (fallback 3)
            FLUX_SCHNELL = {"model": "black-forest-labs/FLUX.1-schnell", "provider": "fal-ai", "timeout": 10.0}
            FLUX_DEV = {"model": "black-forest-labs/FLUX.1-dev", "provider": None, "timeout": 20.0}
            SD35 = {"model": "stabilityai/stable-diffusion-3.5-large", "provider": None, "timeout": 20.0}
            SD15 = {"model": "stable-diffusion-v1-5/stable-diffusion-v1-5", "provider": None, "timeout": 20.0}

            if model_choice in ('sdxl', 'sdxl_base'):
                attempts = [SD35, SD15]
            elif model_choice == 'sd15':
                attempts = [SD15]
            else:
                # 'flux_schnell' and any legacy/unknown choice: full chain
                attempts = [FLUX_SCHNELL, FLUX_DEV, SD35, SD15]

            for attempt in attempts:
                try:
                    m_id = attempt["model"]
                    prov = attempt["provider"]
                    timeout_val = attempt["timeout"]
                    print(f"Attempting image generation: {m_id} (provider={prov})...")

                    if prov:
                        client = InferenceClient(provider=prov, token=hf_token, timeout=timeout_val)
                    else:
                        client = InferenceClient(token=hf_token, timeout=timeout_val)

                    pil_img = client.text_to_image(prompt, model=m_id)

                    # Convert PIL image to bytes
                    img_byte_arr = io.BytesIO()
                    pil_img.save(img_byte_arr, format='PNG')
                    candidate = img_byte_arr.getvalue()

                    # Magic-bytes check: accept only real PNG/JPEG data,
                    # never an HTML error page masquerading as an image
                    if not (candidate.startswith(b'\x89PNG') or candidate.startswith(b'\xff\xd8')):
                        raise ValueError("Response failed PNG/JPEG magic-bytes validation")

                    image_content = candidate
                    model_used = m_id
                    is_fallback = (m_id != attempts[0]["model"])
                    break
                except Exception as e:
                    print(f"InferenceClient failed for {attempt['model']}: {e}")


        # If HF failed or no token was configured
        if not image_content:
            try:
                import urllib.parse
                print("Using Pollinations.ai image generator fallback...")
                ...
                # HTTP 200 + PNG/JPEG magic-bytes check
                if (response.status_code == 200 and len(response.content) > 100
                        and (response.content.startswith(b'\x89PNG') or response.content.startswith(b'\xff\xd8'))):
                    image_content = response.content
                    model_used = "pollinations.ai/flux"
                    is_fallback = (model_choice != 'pollinations')
            except Exception as e:
        ...
        if image_content:
            ...
            from django.core.files.base import ContentFile
            default_storage.save(filename, ContentFile(image_content))
        ...
        return Response({'error': 'Image generation failed on all available models and fallbacks.'}, status=500)
